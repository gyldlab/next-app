...
const BASE_DOC_LINKS = [{ label: "Next.js Docs →", href: "https://nextjs.org/docs" }] as const;

const SUPPORT_LINKS = [
  { label: "CLI Repo →", href: "https://github.com/gyldlab/next-app" },
  {
    label: "Report Issue →",
    href: "https://github.com/gyldlab/next-app/issues/new/choose",
  },
  {
...
const ADDON_DOC_LINKS: Readonly<Record<string, HomePageLink>> = {
  elysia: {
    label: "Elysia Docs →",
...
export function generateHomePage(addonIds: readonly string[]): string {
  const docLinks = getDocLinks(addonIds);
  ...
  return [
    ...
    "",
    renderLinkConstant("docLinks", docLinks),
    "",
    renderLinkConstant("supportLinks", SUPPORT_LINKS),
    "",
    ...renderHomeComponentLines(),
    "",
  ].join("\n");
}

function getDocLinks(addonIds: readonly string[]): HomePageLink[] {
  const docLinks: HomePageLink[] = [...BASE_DOC_LINKS];

  for (const addonId of addonIds) {
    const docLink = ADDON_DOC_LINKS[addonId];
    if (docLink) {
...
function renderLinkConstant(constantName: string, links: readonly HomePageLink[]): string {
  const serializedLinks = links
    .map(
      ({ label, href }) => `  { label: ${JSON.stringify(label)}, href: ${JSON.stringify(href)} },`,
    )
    .join("\n");
...
function renderHomeComponentLines(): string[] {
  return [
...
function renderHeroLines(): string[] {
  return [
    ...
    '              <code className="font-mono tracking-normal text-zinc-950 normal-case dark:text-zinc-100">src/app/page.tsx</code>',
    "            </p>",
    '            <p className="text-base leading-7 text-pretty text-zinc-700 dark:text-zinc-200">',
    "              This scaffold ships with production-ready defaults. Use the docs below for your stack, and if the CLI needs a fix or another add-on, raise it on GitHub.",
    "            </p>",
    "          </div>",
    "        </div>",
    "      </section>",
