...
import { getBaseTemplates, getAddons, type BaseTemplateInfo, type AddonInfo } from "../core/templates.js";
        ...
        return (
          <Text key={`text-${rowIndex}`}>
            {chars.map((char, colIndex) => {
              ...
              const totalColors = config.text.colors.length;
              const colorIndex = ((textOffset - diagonalIndex) % totalColors + totalColors) % totalColors;
              const color = config.text.colors[colorIndex]!;
...
const TemplateSelector: React.FC<TemplateSelectorProps> = ({ templates, selectedIndex }) => {
  return (
    <Box flexDirection="column" marginTop={1}>
      <Text bold color="cyan">Select a template (↑/↓ then Enter):</Text>
      <Box flexDirection="column" marginTop={1}>
...
const ListMode: React.FC<ListModeProps> = ({ logoOffset, textOffset, templates, addons, config }) => {
  const { exit } = useApp();
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text> </Text>
      <Text bold color="cyan">Base templates:</Text>
      {templates.map((t) => (
      ...
      <Text> </Text>
      <Text bold color="cyan">Add-ons:</Text>
      {addons.map((a) => (
...
const InteractiveApp: React.FC<InteractiveAppProps> = ({ projectName, install, mode, templates, addons, config }) => {
  const { exit } = useApp();
  ...
  // List mode
  if (mode === "list") {
    return <ListMode logoOffset={logoOffset} textOffset={textOffset} templates={templates} addons={addons} config={config} />;
  }
...
export async function runInteractiveMode(
  ...
  const [templates, addons] = await Promise.all([
    getBaseTemplates(),
    getAddons(),
  ]);
