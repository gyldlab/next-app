 ...
 *   "BT" - Bottom to Top ↑
 */
export type AnimationDirection = "TLBR" | "TRBL" | "BLTR" | "BRTL" | "LR" | "RL" | "TB" | "BT";
...
export function calculateDiagonalIndex(
  ...
  switch (direction) {
    ...
    case "TLBR":
      return row + Math.floor(col / bandWidth);

    // TRBL ↙: top-right=low, bottom-left=high
    case "TRBL":
      return row + Math.floor((totalCols - col) / bandWidth);

    // BLTR ↗: bottom-left=low, top-right=high
    case "BLTR":
      return totalRows - row + Math.floor(col / bandWidth);

    // BRTL ↖: bottom-right=low, top-left=high
    case "BRTL":
      return totalRows - row + Math.floor((totalCols - col) / bandWidth);

    // LR →: left=low, right=high
    case "LR":
      return Math.floor(col / bandWidth);

    // RL ←: right=low, left=high
    case "RL":
      return Math.floor((totalCols - col) / bandWidth);
